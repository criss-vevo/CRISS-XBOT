...
fredi({
  ...
  if (!arg || arg.length === 0) {
    const replyText = "Example Usage: .code 25752xxxxxx.";
    return repondre(replyText);
  }

  try {
    // Notify user that pairing is in progress
    const replyText = "*Wait Criss Xbot is getting your pair code ✅...*";
...
*Criss Xbot Element Information:*
🚀 *Name:* ${data.name}
🚀 *Symbol:* ${data.symbol}
...
Regards ${conf.BOT} `;
    ...
    await zk.sendMessage(dest, {
      ...
      contextInfo: {
        externalAdReply: {
          title: "CRISS-XBOT ELEMENT INFORMATION",
...
`;
    ...
    await zk.sendMessage(dest, {
      ...
      contextInfo: {
        externalAdReply: {
          title: "CRISS-XBOT GITHUB USER INFO",
